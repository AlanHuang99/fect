...
fect_boot <- function(
  ...
  if (!is.null(group)) {
    ...
    for (sub.name in group.output.name) {
      ...
      if (hasRevs == 1) {
        ...
        group.atts.off.boot[[sub.name]] <- matrix(
          0,
          length(subgroup.time.off),
          nboots
        )
      }
      if (placeboTest) {
  ...
  if (binary == TRUE & vartype == "parametric") {
    one.nonpara <- function(num = NULL) {
      ...
      Y.boot.new <- matrix(
        sapply(1:length(c(Y.fit)), function(i) {
    ...
    one.nonpara <- function(num = NULL) {
      ...
      Y.boot[, (Ntr + 1):length(id.boot)] <- Y.boot[,
        (Ntr + 1):length(id.boot)
      ] +
        error.co.boot
      X.boot <- NULL
      if (p > 0) {
      ...
      if (is.null(num)) {
        if (is.null(cl)) {
          ...
          for (kk in 1:length(cl.boot.uni)) {
            boot.id <- c(
              boot.id,
              rep(which(cl.id == cl.boot.uni[kk]), cl.boot.count[kk])
            )
          }
        }
  ...
  if (sum(is.na(c(att.avg.boot))) > 0) {
    ...
    if (!is.null(balance.period)) {
      ...
      if (!is.null(placebo.period) & placeboTest == TRUE) {
        balance.att.placebo.boot <- t(as.matrix(balance.att.placebo.boot[,
          -boot.rm
        ]))
      }
    }
    ...
    if (!is.null(group)) {
      ...
      for (sub.name in group.output.name) {
        group.atts.boot[[sub.name]] <- as.matrix(group.atts.boot[[sub.name]][,
          -boot.rm
        ])
        if (hasRevs == 1) {
          group.atts.off.boot[[sub.name]] <- as.matrix(group.atts.off.boot[[
            sub.name
          ]][, -boot.rm])
        }
        if (placeboTest) {
          group.att.placebo.boot[[
            sub.name
          ]] <- t(as.matrix(group.att.placebo.boot[[sub.name]][, -boot.rm]))
        }
        if (carryoverTest) {
          group.att.carryover.boot[[
            sub.name
  ...
  if (vartype != "parametric") {
    ...
    # --- Input Checks ---
    if (!exists("boot.out")) {
      stop("List 'boot.out' required for non-parametric vartype.")
    }
    if (exists("boot.out") && !is.list(boot.out)) {
      stop("'boot.out' must be a list.")
    }
    if (!exists("T.on") || !exists("N")) {
      stop("'T.on' and 'N' must be defined.")
    }
    T_val <- tryCatch(
      if (is.matrix(T.on) || is.data.frame(T.on)) nrow(T.on) else length(T.on),
      error = function(e) 0
    )
    if (!is.numeric(T_val) || T_val < 0) {
      stop("'T.on' does not yield a valid number of time periods.")
    }
    if (!is.numeric(N) || N < 0) {
      stop("'N' must be a non-negative numeric value.")
    }
    if (!exists("D")) {
      stop("Reference treatment matrix 'D' must be defined.")
    }
    if (!is.matrix(D) && !is.data.frame(D)) {
      stop("'D' must be a matrix or data frame.")
    }
    if (!is.numeric(D)) {
      warning("Coercing reference matrix D to numeric.")
      mode(D) <- "numeric"
    }
    if (nrow(D) != T_val || (N > 0 && ncol(D) != N)) {
      stop("Dimensions of 'D' do not match T.on and N.")
    }
    ...
    if (vartype == "jackknife" && N <= 1) {
      ...
      out$Y.avg <- data.frame(
        period = numeric(0),
        treated = numeric(0),
        counterfactual = numeric(0),
        lower.tr = numeric(0),
        upper.tr = numeric(0),
        lower90.tr = numeric(0),
        upper90.tr = numeric(0),
        lower.ct = numeric(0),
        upper.ct = numeric(0),
        lower90.ct = numeric(0),
        upper90.ct = numeric(0)
      )
    ...
    if (T_val == 0 || N == 0) {
      warning(paste(
        "Input data has zero ",
        if (T_val == 0) "time periods" else "units",
        ". Returning empty Y.avg.",
        sep = ""
      ))
      if (!exists("out")) {
        out <- list()
    ...
    first_treat_period_D0 <- if (N > 0) {
      calculate_first_treatment(D0, N)
    } else {
      integer(0)
    }
    ...
    if (Ntr == 0) {
      ...
      unique_adoption_times_D0 <- unique(first_treat_period_D0[
        treated_units_idx_D0
      ])
      is_staggered <- length(unique_adoption_times_D0) > 1
      if (!is_staggered) {
        common_G_D0 <- unique_adoption_times_D0
      } else {
        common_G_D0 <- NA
      }
    }
    ...
    if (!is.null(rownames(D0))) {
      cal_labels_from_rows <- tryCatch(
        as.numeric(rownames(D0)),
        warning = function(w) NULL
      )
      if (
        !is.null(cal_labels_from_rows) &&
          !anyNA(cal_labels_from_rows) &&
          length(cal_labels_from_rows) == T_val
      ) {
        calendar_periods_labels <- cal_labels_from_rows
      } else {
        warning("Rownames of D not valid numeric labels. Using 1:T.")
      }
    }
    ...
    if (is_staggered) {
      ...
      min_event_time <- -(T_val - 1) + 1
      max_event_time <- T_val
      event_times_range <- min_event_time:max_event_time
      n_event_times <- length(event_times_range)
      event_time_matrix_D0 <- outer(
        seq.int(T_val),
        first_treat_period_D0,
        FUN = "-"
      ) +
        1

      # Calculate Replicates if Bootstrap/Jackknife
      event_repl_cf <- matrix(
        NA_real_,
        nrow = n_event_times,
        ncol = n_replicates
      )
      event_repl_tr <- matrix(
        NA_real_,
        nrow = n_event_times,
        ncol = n_replicates
      )
        ...
        for (b in seq_len(n_replicates)) {
          ...
          if (
            is.null(repl_data) ||
              ...
              is.null(repl_data$D) ||
              is.null(repl_data$Y) ||
              is.null(repl_data$Y.ct)
          ) {
            next
          }
          D_b <- repl_data$D
          Y_b <- repl_data$Y
          Yct_b <- repl_data$Y.ct
          if (!is.matrix(D_b)) {
            D_b <- as.matrix(D_b)
          }
          if (!is.matrix(Y_b)) {
          ...
          if (!is.matrix(Yct_b)) {
            Yct_b <- as.matrix(Yct_b)
          }
          if (!is.numeric(D_b)) {
            mode(D_b) <- "numeric"
          }
          if (!is.numeric(Y_b)) {
            warning(paste("Coercing replicate", b, "Y to numeric."))
            mode(Y_b) <- "numeric"
          }
          if (!is.numeric(Yct_b)) {
            warning(paste("Coercing replicate", b, "Y.ct to numeric."))
            mode(Yct_b) <- "numeric"
          }
          ...
          if (
            nrow(D_b) != T_val ||
              nrow(Y_b) != T_val ||
              nrow(Yct_b) != T_val ||
              (N_b > 0 && (ncol(Y_b) != N_b || ncol(Yct_b) != N_b))
          ) {
            warning(
              paste(
                "Replicate",
                b,
                ": Dimension mismatch with T_val. Skipping replicate."
              ),
              call. = FALSE
            )
            next
          ...
          if (N_b == 0) {
            next
          } # If a replicate has 0 units, skip it (relevant if N > 0 originally)

          Y_b[is.na(Yct_b)] <- NA

          first_treat_period_b <- calculate_first_treatment(D_b, N_b)
          event_time_matrix_b <- outer(
            seq.int(T_val),
            first_treat_period_b,
            FUN = "-"
          ) +
            1
      ...
      if (n_replicates > 0) {
        for (b in seq_len(n_replicates)) {
          ...
          if (vartype == "jackknife") {
            expected_N_b <- N - 1
            if (N_b == N - 1 && n_replicates == N && N > 0) {
              # N > 0 for jackknife to make sense
              ...
              # Only warn if N > 0, jackknife on N=0 is already handled.
              warning(
                paste(
                  "Replicate",
                  b,
                  ": Jackknife replicate has",
                  N_b,
                  "columns (expected N-1=",
                  N - 1,
                  ") or n_replicates (",
                  n_replicates,
                  ") != N (",
          ...
          if (N > 0 && N_b != expected_N_b) {
            # Check for N > 0 before N_b != expected_N_b
            warning(
              paste(
                "Replicate",
                b,
          ...
          if (N_b == 0 && N > 0) {
            next
          } # If N > 0, but this replicate has 0 units, skip. If N=0, N_b=0 is expected.

          Y_b[is.na(Yct_b)] <- NA

          # Get the column indices *in the current replicate D_b* that correspond to these target units
          replicate_cols_for_target_units <- which(
            colSums(D_b == 1, na.rm = TRUE) > 0
          )
          ...
          for (t in seq.int(T_val)) {
            ...
            if (Ntr == 0) {
              # No treated units in D0, so effectively averaging "control" outcomes for "placebo" treated group
            } else if (!is.na(common_G_D0) && t < common_G_D0) {
              # Pre-treatment period for non-staggered
              cols_to_avg <- replicate_cols_for_target_units
            } else if (!is.na(common_G_D0) && t >= common_G_D0) {
              # Post-treatment period for non-staggered
              # Only average over units that are actually treated (D_b[t,col]==1) in this replicate at this time
              cols_to_avg <- D_b[t, ] == 1
            } else if (is.na(common_G_D0) && Ntr > 0) {
              # Should not happen if is_staggered is FALSE and Ntr > 0
              warning(
                paste(
                  "Replicate",
                  b,
                  ": In non-staggered case with Ntr > 0, but common_G_D0 is NA. Skipping time",
                  t,
                  "for this replicate."
                ),
                call. = FALSE
    ...
    if (vartype == "bootstrap") {
      if (n_replicates > 0 && num_periods_out > 0) {
        if (
          ...
          warning(
            "Bootstrap replicate matrix dimensions mismatch with number of output periods. Skipping CI."
          )
        } else {
        ...
        if (num_periods_out > 0) {
          warning(
            "Cannot calculate bootstrap CIs: No replicates remain or available."
          )
        }
      } # num_periods_out > 0 condition added to warning
    } else if (vartype == "jackknife") {
      if (n_replicates > 1 && num_periods_out > 0) {
        # Jackknife needs n_replicates > 1 (i.e., N > 1)
        if (
          nrow(repl_tr) != num_periods_out || nrow(repl_cf) != num_periods_out
        ) {
          warning(
            "Jackknife replicate matrix dimensions mismatch with number of output periods. Skipping CI."
          )
        } else {
        ...
        if (num_periods_out > 0) {
          warning(
            "Cannot calculate jackknife CIs: Need >1 replicate (N > 1) or no periods remain."
          )
        }
      } # num_periods_out > 0 condition added
    } else {
      stop(
        "Internal error: Invalid 'vartype' for non-parametric CI calculation."
      )
    }
    ...
    ensure_numeric_matrix_cols <- function(mat, expected_rows) {
      ...
      if (!is.numeric(mat)) {
        # Coerce if not numeric (e.g. character matrix)
        mat_num <- matrix(
          as.numeric(mat),
          nrow = nrow(mat),
          ncol = ncol(mat),
    ...
    if (num_periods_out > 0) {
      ...
      theta_tr_vec <- if (length(theta_tr) == num_periods_out) {
        as.numeric(theta_tr)
      } else {
        rep(NA_real_, num_periods_out)
      }
      theta_cf_vec <- if (length(theta_cf) == num_periods_out) {
        as.numeric(theta_cf)
      } else {
        rep(NA_real_, num_periods_out)
      }
      ...
      out$Y.avg <- data.frame(
        ...
        lower.tr = ci_tr95[, "lower"],
        upper.tr = ci_tr95[, "upper"],
        lower90.tr = ci_tr90[, "lower"],
        upper90.tr = ci_tr90[, "upper"],
        lower.ct = ci_cf95[, "lower"],
        upper.ct = ci_cf95[, "upper"],
        lower90.ct = ci_cf90[, "lower"],
        upper90.ct = ci_cf90[, "upper"]
      )
    } else {
      # This case handles T_val=0 or N=0 (via early return) or if periods somehow becomes length 0
      out$Y.avg <- data.frame(
        period = numeric(0),
        treated = numeric(0),
        counterfactual = numeric(0),
  ...
  if (vartype == "jackknife") {
    ...
    att.bound <- cbind(
      att + qnorm(alpha) * att.j$se,
      att + qnorm(1 - alpha) * att.j$se
    )
    ...
    est.eff.calendar.fit <- cbind(
      calendar.eff.fit,
      eff.calendar.fit.j$se,
      eff.calendar.fit.j$CI.l,
      eff.calendar.fit.j$CI.u,
      eff.calendar.fit.j$P,
      calendar.N
    )
    colnames(est.eff.calendar.fit) <- c(
      "ATT-calendar Fitted",
      "S.E.",
      "CI.lower",
      "CI.upper",
    ...
    if (hasRevs == 1) {
      ...
      att.off.bound <- cbind(
        att.off + qnorm(alpha) * att.off.j$se,
        att.off + qnorm(1 - alpha) * att.off.j$se
      )
    ...
    if (!is.null(balance.period)) {
      ...
      est.balance.avg <- t(as.matrix(c(
        balance.avg.att,
        balance.avg.att.j$se,
        balance.avg.att.j$CI.l,
        balance.avg.att.j$CI.u,
        balance.avg.att.j$P
      )))
      colnames(est.balance.avg) <- c(
        "ATT.avg",
        "S.E.",
        "CI.lower",
        "CI.upper",
      ...
      balance.att.bound <- cbind(
        balance.att + qnorm(alpha) * balance.att.j$se,
        balance.att + qnorm(1 - alpha) * balance.att.j$se
      )
      colnames(balance.att.bound) <- c("CI.lower", "CI.upper")
      rownames(balance.att.bound) <- out$balance.time

      if (!is.null(placebo.period) & placeboTest == TRUE) {
        balance.att.placebo.j <- jackknifed(
          balance.att.placebo,
        ...
        est.balance.placebo <- t(as.matrix(c(
          balance.att.placebo,
          balance.att.placebo.j$se,
          balance.att.placebo.j$CI.l,
          balance.att.placebo.j$CI.u,
          balance.att.placebo.j$P
        )))
    ...
    if (!is.null(W)) {
      ...
      att.W.bound <- cbind(
        att.on.W + qnorm(alpha) * att.on.W.j$se,
        att.on.W + qnorm(1 - alpha) * att.on.W.j$se
      )
      ...
      if (hasRevs == 1) {
        ...
        att.off.W.bound <- cbind(
          att.off.W + qnorm(alpha) * att.off.W.j$se,
          att.off.W + qnorm(1 - alpha) * att.off.W.j$se
        )
        colnames(att.off.W.bound) <- c("CI.lower", "CI.upper")
        rownames(att.off.W.bound) <- out$time.off

        if (!is.null(carryover.period) & carryoverTest == TRUE) {
          att.carryover.W.j <- jackknifed(
            att.carryover.W,
    ...
    att.avg.j <- jackknifed(
      att.avg,
      att.avg.boot,
      alpha,
      quantile.CI = quantile.CI
    )
    est.avg <- t(as.matrix(c(
      att.avg,
      att.avg.j$se,
      att.avg.j$CI.l,
      att.avg.j$CI.u,
    ...
    if (!is.null(carryover.period) & carryoverTest == TRUE) {
      ...
      est.carryover <- t(as.matrix(c(
        att.carryover,
        att.carryover.j$se,
        att.carryover.j$CI.l,
        att.carryover.j$CI.u,
        att.carryover.j$P,
        att.carryover.bound
      )))
    ...
    if (!is.null(group)) {
      ...
      for (sub.name in group.output.name) {
        ...
        if (dim(subgroup.atts.boot)[1] > 0) {
          ...
          rownames(subgroup.att.bound) <- group.output.origin[[
            sub.name
          ]]$time.on
        }
        ...
        if (hasRevs == 1) {
          ...
          if (dim(subgroup.atts.off.boot)[1] > 0) {
            ...
            subgroup.est.att.off <- cbind(
              subgroup.atts.off,
              subgroup.att.off.j$se,
              subgroup.att.off.j$CI.l,
              subgroup.att.off.j$CI.u,
              subgroup.att.off.j$P,
            ...
            rownames(subgroup.est.att.off) <- group.output.origin[[
              sub.name
            ]]$time.off
            ...
            rownames(subgroup.att.off.bound) <- group.output.origin[[
              sub.name
            ]]$time.off
          }
        }
        ...
        if (placeboTest) {
          ...
          if (length(subgroup.att.placebo) > 0) {
            subgroup.att.placebo.j <- jackknifed(
            ...
            att.placebo.bound <- c(
              ...
              subgroup.att.placebo +
                qnorm(1 - alpha) * subgroup.att.placebo.j$se
            )
        ...
        if (carryoverTest) {
          subgroup.att.carryover <- group.output.origin[[
            sub.name
          ]]$att.carryover
          if (length(subgroup.att.carryover) > 0) {
            subgroup.att.carryover.j <- jackknifed(
            ...
            att.carryover.bound <- c(
              subgroup.att.carryover +
                qnorm(alpha) * subgroup.att.carryover.j$se,
              subgroup.att.carryover +
                qnorm(1 - alpha) * subgroup.att.carryover.j$se
            )
    ...
    if (hasRevs == 1) {
      ...
      if (quantile.CI == FALSE) {
        CI.att.off <- cbind(
          att.off - se.att.off * qnorm(1 - alpha / 2),
          att.off + se.att.off * qnorm(1 - alpha / 2)
        )
    ...
    if (!is.null(T.on.carry)) {
      se.carry.att <- apply(carry.att.boot, 1, function(vec) {
        sd(vec, na.rm = TRUE)
      })
      if (quantile.CI == FALSE) {
        CI.carry.att <- cbind(
    ...
    if (!is.null(balance.period)) {
      se.balance.att <- apply(balance.att.boot, 1, function(vec) {
        sd(vec, na.rm = TRUE)
      })
      if (quantile.CI == FALSE) {
    ...
    if (!is.null(W)) {
      ...
      if (hasRevs == 1) {
        # att.off.W.boot
        se.att.off.W <- apply(att.off.W.boot, 1, function(vec) {
          sd(vec, na.rm = TRUE)
        })
        if (quantile.CI == FALSE) {
          CI.att.off.W <- cbind(
    ...
    if (quantile.CI == FALSE) {
      CI.avg <- c(
        att.avg - se.avg * qnorm(1 - alpha / 2),
        att.avg + se.avg * qnorm(1 - alpha / 2)
      )
    ...
    se.eff.calendar <- apply(calendar.eff.boot, 1, function(vec) {
      sd(vec, na.rm = TRUE)
    })
    if (quantile.CI == FALSE) {
      CI.eff.calendar <- cbind(
        calendar.eff - se.eff.calendar * qnorm(1 - alpha / 2),
        calendar.eff + se.eff.calendar * qnorm(1 - alpha / 2)
      )
    ...
    se.eff.calendar.fit <- apply(calendar.eff.fit.boot, 1, function(vec) {
      sd(vec, na.rm = TRUE)
    })
    if (quantile.CI == FALSE) {
      CI.eff.calendar.fit <- cbind(
        calendar.eff.fit - se.eff.calendar.fit * qnorm(1 - alpha / 2),
        calendar.eff.fit + se.eff.calendar.fit * qnorm(1 - alpha / 2)
      )
      pvalue.eff.calendar.fit <- (1 -
    ...
    if (p > 0) {
      ...
      if (quantile.CI == FALSE) {
        CI.beta <- cbind(
          c(beta) - se.beta * qnorm(1 - alpha / 2),
          c(beta) + se.beta * qnorm(1 - alpha / 2)
        )
      ...
      if (binary == TRUE) {
        ...
        se.marginal <- apply(marginal.boot, 1, function(vec) {
          sd(vec, na.rm = TRUE)
        })
        CI.marginal <- cbind(
    ...
    if (!is.null(group)) {
      ...
      for (sub.name in group.output.name) {
        ...
        subgroup.att.bound <- NULL
        if (dim(subgroup.atts.boot)[1] > 0) {
          subgroup.se.att <- apply(subgroup.atts.boot, 1, function(vec) {
            sd(vec, na.rm = TRUE)
          })
          if (quantile.CI == FALSE) {
            ...
            subgroup.pvalue.att <- (1 -
              pnorm(abs(subgroup.atts / subgroup.se.att))) *
              2
            subgroup.att.bound <- cbind(
        ...
        if (hasRevs == 1) {
          ...
          if (dim(subgroup.atts.off.boot)[1] > 0) {
            subgroup.se.att.off <- apply(
              subgroup.atts.off.boot,
              1,
              function(vec) sd(vec, na.rm = TRUE)
            )
            if (quantile.CI == FALSE) {
          ...
          if (length(subgroup.att.placebo) > 0) {
            ...
            if (quantile.CI == FALSE) {
              subgroup.CI.placebo <- c(
                subgroup.att.placebo -
                  subgroup.se.placebo * qnorm(1 - alpha / 2),
                subgroup.att.placebo +
                  subgroup.se.placebo * qnorm(1 - alpha / 2)
              )
          ...
          if (length(subgroup.att.carryover) > 0) {
            ...
            if (quantile.CI == FALSE) {
              subgroup.CI.carryover <- c(
                subgroup.att.carryover -
                  subgroup.se.carryover * qnorm(1 - alpha / 2),
                subgroup.att.carryover +
                  subgroup.se.carryover * qnorm(1 - alpha / 2)
              )
              subgroup.CI.carryover.bound <- c(
                subgroup.att.carryover -
                  subgroup.se.carryover * qnorm(1 - alpha),
                subgroup.att.carryover +
                  subgroup.se.carryover * qnorm(1 - alpha)
              )
              subgroup.pvalue.carryover <- (1 -
  ...
  if (!is.null(balance.period)) {
    ...
    if (!is.null(placebo.period) & placeboTest == TRUE) {
      result <- c(
        result,
        list(
          est.balance.placebo = est.balance.placebo,
          balance.att.placebo.boot = balance.att.placebo.boot
        )
      )
    }
  }
  ...
  if (!is.null(carryover.period) & carryoverTest == TRUE) {
    result <- c(
      result,
      list(
        est.carryover = est.carryover,
        att.carryover.boot = att.carryover.boot
      )
    )
  }
...
jackknifed <- function(
  ...
  if (quantile.CI == FALSE) {
    ...
    CI <- t(apply(y, 1, function(vec) {
      quantile(vec, c(0.05 / 2, 1 - 0.05 / 2), na.rm = TRUE)
    }))
    CI.l <- CI[, 1]
    CI.u <- CI[, 2]
  }
